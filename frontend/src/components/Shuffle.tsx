...
const Shuffle: React.FC<ShuffleProps> = ({
  ...
  useGSAP(
    () => {
      ...
      const customSplitText = (container: HTMLElement) => {
      ...
      const build = () => {
        ...
        const chars = customSplitText(el);
        wrappersRef.current = [];
      ...
      const play = () => {
        ...
        const tl = gsap.timeline({
          ...
          onRepeat: () => {
            ...
            if (isVertical) {
              gsap.set(strips, { y: (_, t) => parseFloat((t as HTMLElement).getAttribute('data-start-y') || '0') });
            } else {
              gsap.set(strips, { x: (_, t) => parseFloat((t as HTMLElement).getAttribute('data-start-x') || '0') });
            }
            onShuffleComplete?.();
          },
          onComplete: () => {
      ...
      return () => {
        ...
        teardown();
        setReady(false);
        if (ref.current && originalHTMLRef.current) {
