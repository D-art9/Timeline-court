...
interface TouchTextureInstance {
...
const createTouchTexture = (): TouchTextureInstance => {
  const size = 64;
  ...
  const drawPoint = (p: TouchPoint) => {
    ...
    intensity *= p.force;
    const colorCode = `${((p.vx + 1) / 2) * 255}, ${((p.vy + 1) / 2) * 255}, ${intensity * 255}`;
    const offset = size * 5;
    ctx.shadowOffsetX = offset;
    ctx.shadowOffsetY = offset;
    ctx.shadowBlur = radius;
    ctx.shadowColor = `rgba(${colorCode},${0.22 * intensity})`;
    ctx.beginPath();
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      let touch: TouchTextureInstance | undefined;
      ...
      let raf = 0;
      const animate = () => {
        if (document.hidden || (autoPauseOffscreen && !visibilityRef.current.visible)) {
